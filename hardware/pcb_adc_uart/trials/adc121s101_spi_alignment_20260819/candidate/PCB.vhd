...
architecture Behavioral of PCB is
    ...
    constant frame_ticks : integer := 63;
    ...
    type adc_state_t is (
        idle, sclk_rise, sclk_fall, frame_close,
        add, div, wait1, wait2, emit, frame_wait
    );
...
begin
    ...
    adc_p : process(adc_clk)
        ...
        variable frame_v    : std_logic_vector(15 downto 0);
    begin
        if rising_edge(adc_clk) then
            ...
            if reset_in = '1' then
                ...
                case adc_state is
                    when idle =>
                        -- Start a fixed 63-adc_clk conversion frame.  CS is
                        -- asserted while SCLK remains low; the first SCLK
                        -- transition occurs one adc_clk later, satisfying the
                        -- ADC121S101 CS-to-SCLK setup interval.
                        adc_state     <= sclk_rise;
                        spi_clk_r     <= '0';
                        cs_r          <= '0';
                        frame_tick    <= 0;
                    ...
                    when sclk_rise =>
                        -- The ADC changes SDATA after falling SCLK edges.  The
                        -- following falling-edge state captures the bit that
                        -- has therefore been stable for a complete SCLK cycle.
                        spi_clk_r <= '1';
                        cs_r      <= '0';
                    ...
                    when sclk_fall =>
                        -- Read MISO before lowering the physical SCLK pin.  On
                        -- falling edge 16, frame_v contains the newly captured
                        -- DB0; using frame_shift here would omit that last bit.
                        frame_v := frame_shift(14 downto 0) & spi_miso;
                        frame_shift <= frame_v;
                        spi_clk_r   <= '0';
                        cs_r        <= '0';
                        frame_tick  <= frame_tick + 1;

                        if fall_count = 15 then
                            data_buf   <= frame_v(11 downto 0);
                            fall_count <= 0;
                            adc_state  <= frame_close;
                        else
                    ...
                    when frame_close =>
                        -- Keep CS low through falling edge 16, then close the
                        -- frame on its own adc_clk edge to avoid pin-skew races.
                        spi_clk_r  <= '0';
                        cs_r       <= '1';
                        frame_tick <= frame_tick + 1;

                        if discard_first = '1' then
                            -- TI specifies the first conversion after leaving
                            -- shutdown/power-up as unusable.  Do not let the
                            -- dummy frame enter either the sum or sample count.
                            discard_first <= '0';
                            accum_sum     <= 0;
                            decim_count   <= 0;
                            adc_state     <= frame_wait;
                        else
                            adc_state <= add;
                        end if;
                    ...
                    when frame_wait =>
                        ...
                        if frame_tick = frame_ticks - 1 then
                            -- Begin the next frame on the fixed 63-tick
                            -- cadence.  SCLK stays low for one setup tick.
                            cs_r        <= '0';
                            frame_tick  <= 0;
                            fall_count  <= 0;
                            frame_shift <= (others => '0');
